...
def smooth_filter(array,index,n,dense):  # return weighted moving average point for ndays
    ...
    if len(array) > index and index >= n-1 :
        ...
        for i in range(0,len(window)):
            ...
            weight_sum += weight
        smooth_avg = value_sum/weight_sum
        avg = sum(window)/len(window)
        return [smooth_avg,avg]
...
for i in range(0,len(data)):
    value = smooth_filter(data,i,n,dense)
    if value != -1 :
...
plt.plot(filtered_date,filtered_data,color='red',label='smooth moving avg filter')
plt.title('smooth moving average filter')
plt.legend()
plt.show()
